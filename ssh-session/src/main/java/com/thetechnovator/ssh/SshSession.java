...
public class SshSession {
	...
	public synchronized void connect() throws SshSessionlException {
		...
		try {
			...
			isOpen = true;
			AbstractCommandExecutor cmd=new AbstractCommandExecutor(this) {
				
			};
			cmd.sendCommand("echo 'Hello World'");
	...
	private void doSwitchUser(String cmd) throws SshSessionlException {
		...
		SshCommand sshCmd = new SshCommand(Constants.CMD_PREFIX+cmd);
		int status = cmdExecutor.exec(sshCmd, cmd);
		if (status != SUCCESS) {
