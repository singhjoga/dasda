...
public abstract class SshInputReader2 {
	...
	public SshInputReader2(SshCommand cmd, SshSession session) {
	...
	public int readCommandOutput(String cmd) throws SshSessionlException {
		...
		boolean start = true;
		boolean commandOutputStart = false;
		status = -1;
		try {
			...
			while (status == -1) {
				...
				sb.append((char) c);
				readLen++;
				if (!commandOutputStart) {
					if (readLen < cmd.length()) {
						// ignore command output
						continue;
					} else if (isCommandOutputEnd(sb, cmd)) {
						...
						commandOutputStart=true;
						continue;
					}
				}
				/*
				 * } else if (readLen == cmd.length()) { if (cmd.endsWith(Constants.CMD_SUFFIX))
				 * { writeCommand(cmd.substring(0, cmd.length() - Constants.CMD_SUFFIX.length())
				 * + System.lineSeparator()); } else { writeCommand(cmd +
				...
				if (c == Constants.GROUP_SEPARATOR) {
					groupReceived = true;
					continue;
				}
				if (groupReceived && c == Constants.RECORD_SEPARATOR) {
	...
	private boolean isCommandOutputEnd(StringBuilder buf, String cmd) {
	...
	public static void flushBuffer(InputStream in) throws IOException {
		...
		// System.out.println("Flush buffer end");
	}
	...
	private static final class InactivityCheck extends TimerTask {
		private SshInputReader2 reader;
		private long lastActivityTime = System.currentTimeMillis();

		public InactivityCheck(SshInputReader2 reader) {
			super();
			this.reader = reader;
		}
	...
	private static final class MaxExecutionTimeCheck extends TimerTask {
		...
		public MaxExecutionTimeCheck(SshInputReader2 reader) {
			super();
			this.reader = reader;
		}
